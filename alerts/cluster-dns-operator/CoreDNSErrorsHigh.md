...
3. Check CoreDNS pods if they can reach the upstream nameservers:
    ...
    $ oc exec pod/$COREDNS_POD_NAME -c dns -n openshift-dns -- dig -q $DOMAIN_NAME @$IP_OF_UPSTREAM_NAMESERVER
    ```
...
4. Check the upstream nameservers if they are returning SERVFAIL:
    ...
    $ oc patch dnses.operator.openshift.io/default --patch '{"spec":{"logLevel":"Debug"}}' --type=merge
    # Follow the log streams in proportion to the number of CoreDNS pods
    $ oc logs -c dns -l dns.operator.openshift.io/daemonset-dns=default -n openshift-dns --follow --max-log-requests $NUMBER_OF_COREDNS_PODS --timestamps
    ```
