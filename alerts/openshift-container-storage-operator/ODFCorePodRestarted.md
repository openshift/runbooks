...
A core ODF pod (OSD, MON, MGR, ODF operator, or metrics exporter) has restarted at least once in the last 24 hours while the Ceph cluster is active.
...
# Look for "Last State", OOMKilled, CrashLoopBackOff, etc.
```
3. View logs before crash:
```bash
oc logs <pod-name> -n openshift-storage --previous
```
4. Verify resource limits:
```bash
oc get pod <pod> -n openshift-storage -o jsonpath='{.spec.containers[*].resources}'
```
