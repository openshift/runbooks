...
2. Review the `ClusterLogForwarder` (CLF) configuration:
    ...
    from the alert details.
        - **Non-Lokistack Outputs**: The naming schema is `output_<OUTPUT_NAME>`,
        ...
        - **Lokistack Outputs**: The naming schema is `output_<OUTPUT_NAME>_<INPUT_TYPE>`,
...
Ensure the `ClusterLogForwarder`'s associated `ServiceAccount` is correctly
bound to the `logging-collector-logs-writer` ClusterRole.

##### Query for the ClusterRoleBinding
```bash
$ oc get clusterrolebindings --all-namespaces -o yaml | grep -B 5 -A 5 "name: logging-collector-logs-writer"
```
##### Get detailed information about the ClusterRoleBinding
```bash
$ oc describe clusterrolebinding <NAME-OF-CRB>
```
...
1. Inspect which ClusterRoles the ServiceAccount is bound to:
```bash
$ oc get clusterrolebindings --all-namespaces -o yaml | grep -B 5 -A 5 "name: <SA-NAME>"
```

If ServiceAccount is not bound to the `logging-collecor-logs-writer` ClusterRole:

2. Bind ServiceAccount to ClusterRole
```bash
